...
export default function Profile() {
	...
	const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		if (selectedFile) {
			...
			try {
				const response = await fetch('https://h4-09-fintech-production.up.railway.app/api/user/upload-image', {
					method: 'POST',
					headers: {
			...
			<div className={styles.contentContainer}>
              ...
              <FaRegUser className={styles.defaultIcon} />
				...
				<div className={styles.cardContainer}>
					<Card icon={<FaRegUser />} title='Datos financieros y personales' description='Modifique o complete sus datos' link='/editprofile' />
					<Card icon={<GoGear />} title='Configuración' description='Añade o prueba funciones para evitar vulnerabilidades' link='/configurations' />
					<h2 className={styles.subtitle}>Pantalla y acciones</h2>
					<Card icon={<TbWorld />} title='Idioma' description='Cambia toda la interfaz a tu idioma' link='' />
					<Card icon={<LuMoon />} title='Modo oscuro' description='Para facilitar la visualización de la pantalla' link='' />
					<Card icon={<RxExit />} title='Cerrar sesión' description='O permanecer conectado y cambiar de usuario' link='/' />
				</div>
			</div>
		</div>
	)
