...
export const Dashboard: React.FC = () => {
  ...
  const { loading: loadingRetirarFondos, postData } = usePostDataWithToken(
    `${baseUrl}/api/pagos/retiros`
  );
  ...
  const { data: bancAccount } = useFetchDataWithToken<IBancAccount>(
    `${baseUrl}/api/cuenta-banco`
  );
    ...
    if (monto <= 0) {
      ...
      try {
        const response = await fetch(
          `https://h4-09-fintech-production.up.railway.app/api/pagos/depositos?monto=${monto}`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    if (objetivos.length > 0) {
      setBtnObjetivo("Mis objetivos");
    } else {
      setBtnObjetivo("Cargar objetivos");
    }
  }, [objetivos]);
  ...
  return (
    <div className="dashboard">
      ...
      <div className="retirarFondosContainer">
        <button
          type="button"
          className="retirarfondosButton"
          onClick={handleShowretirarFondosModal}
        >
          Retirar fondos
        </button>
      </div>
          ...
          >
            ...
            <div className="inputContainer">
              ...
              <input
                ...
                id="amount"
                min={1}
                max={user?.currentAmount}
                value={montoARetirar}
                onChange={(e) => setMontoARetirar(Number(e.target.value))}
              />
              ...
              <input
                ...
                id="cbu"
                value={cbuARetirar}
                onChange={(e) => setCbuARetirar(e.target.value)}
              />
            </div>
            <div className="modalButtons">
              <button
                type="button"
                onClick={handleWithdrawFounds}
              ...
              <button
                type="button"
                onClick={handleCancelretirarFondosModal}
            ...
            <p>
              *Esta operacion debe ser confirmada por nuestros administradores{" "}
            </p>
          </div>
        </div>
      )}
            ...
            <Typography id="modal-modal-description" sx={{ mt: 2 }}>
              ...
              <form onSubmit={handleSubmitModal}>
                ...
                <div className="bancAccountinfo">
                  <p>
                    Estos son los datos bancarios de Iupi para que puedas
                    realizar una transferencia desde tu banco.
                  </p>
                ...
                <br />
                <p>
                  *Esta operacion debe ser confirmada por nuestros
                  administradores{" "}
                </p>
              </form>
            </Typography>
