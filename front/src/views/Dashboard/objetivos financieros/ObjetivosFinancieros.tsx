...
export const ObjetivosFinancieros = () => {
	...
	const cargarModal = (id: string, name: string, targetAmount: string): void => {
	...
	const handleGoalUpdate = (e: React.FormEvent<HTMLFormElement>) => {
		...
		fetch(`https://h4-09-fintech-production.up.railway.app/api/user/update_goal`, {
			method: 'PATCH',
			headers: {
				Authorization: `Bearer ${token}`,
				'Content-Type': 'application/json'
			...
			.then(() => {
				setObjetivosList((prevList) => prevList.map((goal) => (goal.goalId === infoModal.id ? { ...goal, ...updatedGoal } : goal)))
				setError(null) // Reiniciar errores
					...
					<article id='goal-list'>
						{objetivosList.map((objetivo) => (
							<div key={objetivo.goalId} className='goal-in-list'>
								...
								<PencilIcon className='iconos-hero' onClick={() => cargarModal(objetivo.goalId, objetivo.name, objetivo.targetAmount)} />
							</div>
						))}
					</article>
				)}
				...
				<Box sx={style}>
					...
					<Typography id='modal-modal-description' sx={{ mt: 2 }}>
						<form onSubmit={handleGoalUpdate}>
							<label htmlFor='name'>
								Nombre:
								<input type='text' name='name' id='name' defaultValue={infoModal.name} />
							</label>
							<label htmlFor='targetAmount'>
								Nuevo costo:
								<input type='number' name='targetAmount' id='targetAmount' placeholder={`Costo actual:${infoModal.targetAmount}`} />
							</label>
